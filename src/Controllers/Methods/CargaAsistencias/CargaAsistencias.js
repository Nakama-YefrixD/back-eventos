...
controller.MetCargarAsistencias = async (req, res) => {
  ...
  try {
    const { exists_data, message, status, workbook } =
      await controller.ValExistsData(file);
    ...
    const { messages_error, add_list_approvals, data } =
      await controller.ValCellsFile(workbook);

    const messages = messages_error.flatMap((mess) =>
      mess.notificaciones.map((notif) => notif.msg)
    );
    ...
    for await (const dat of data) {
      let evento_seleccionado = idseventos.find(
        (idevento) => idevento.codigo == dat.codigo_evento
      );
      ...
      if (!evento_seleccionado) {
        ...
        if (evento) {
          ...
          await prisma.eventos.update({
            ...
            data: {
              cargoasistencia: true,
            },
          });
        }
      }

      let usuario_seleccionado = idsusuarios.find(
        (idusuario) => idusuario.usucodigo_ucs == dat.codigo_estudiante
      );
      ...
      let fechaevento_seleccionado = idsfechaseventos.find(
        (idfechaevento) =>
          idfechaevento.fecha == dat.fecha && idfechaevento.hora == dat.hora
      );
      ...
      if (
        ...
        const user_encontrado = usuarios_certificado.find(
          (user_cert) =>
            user_cert.id_event == evento_seleccionado.id &&
            user_cert.usuid == usuario_seleccionado.usuid
        );
        if (!user_encontrado) {
    ...
    return res.json({
      message:
        "Lo sentimos hubo un error al momento de cargar la data de estudiantes",
      devmsg: error,
      respuesta: false,
    });
  } finally {
...
controller.ValCellsFile = async (workbook) => {
  ...
  for await (const row of rows) {
    if (!row[properties[0]]) {
      ...
      let rows_error = messages_error.findIndex(
        (mes) => mes.columna == columns.ex_tipo_documento
      );
      controller.ValAddMessageLog(
        rows_error,
        messages_error,
        columns.ex_tipo_documento,
        num_row,
        "empty"
      );
    }
    ...
    if (!row[properties[1]]) {
      // add_list_approvals = false
      let rows_error = messages_error.findIndex(
        (mes) => mes.columna == columns.ex_numero_documento
      );
      controller.ValAddMessageLog(
        rows_error,
        messages_error,
        columns.ex_numero_documento,
        num_row,
        "empty"
      );
    }
    ...
    if (!row[properties[2]]) {
      // add_list_approvals = false
      let rows_error = messages_error.findIndex(
        (mes) => mes.columna == columns.ex_nombre
      );
      controller.ValAddMessageLog(
        rows_error,
        messages_error,
    ...
    if (!row[properties[3]]) {
      // add_list_approvals = false
      let rows_error = messages_error.findIndex(
        (mes) => mes.columna == columns.ex_apellido_paterno
      );
      controller.ValAddMessageLog(
        rows_error,
        messages_error,
        columns.ex_apellido_paterno,
        num_row,
        "empty"
      );
    }
...
controller.ValAddMessageLog = (
  ...
  if (rows_error == -1) {
    ...
    if (type != "distributor not found") {
      index_type_error = messages_error[rows_error]["notificaciones"].findIndex(
        (typ) => typ.type == type
      );
    } else {
      index_type_error = messages_error[rows_error]["notificaciones"].findIndex(
    ...
    if (index_type_error == -1) {
      ...
      messages_error[rows_error]["notificaciones"][index_type_error][
        "rows"
      ].push(num_row + 1);
    }
  }
